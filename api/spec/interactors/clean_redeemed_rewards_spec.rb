describe CleanRedeemedRewards do
  ...
  let(:interactor_context) do
    interactor.call(
      redeemed_rewards: SetRedeemedRewards.call(user: current_user)['redeemed_rewards']
    )
  end
  ...
  context 'when a history contains redeemed discounts' do
    ...
    it 'displays ticket discounts as rewards' do
      expect(interactor_context.redeemed_rewards.length).to eq(0)
    end
  end
end
